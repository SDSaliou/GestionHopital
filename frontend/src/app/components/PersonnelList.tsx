...
const PersonnelList: React.FC<PersonnelListProps> = ({ personnels, fetchPersonnels }) => {
  ...
  // Filtrage des personnels
  const filteredPersonnels = Array.isArray(personnels)
    ? personnels.filter((personnel) =>
        personnel.nom.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
        personnel.codePersonnel.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
        personnel.contact.toLowerCase().includes(debouncedSearchTerm.toLowerCase())
      )
    : [];

  // Pagination
  const totalPages = Math.ceil(filteredPersonnels.length / personnelsPerPage);
  ...
  const currentPersonnels = filteredPersonnels.slice(indexOfFirstPersonnel, indexOfLastPersonnel);
  ...
  const handleUpdate = async () => {
    if (UPpersonnel) {
      try {
        await axios.put(
          `https://gestion-hopital-api.vercel.app/personnels/${UPpersonnel._id}`,
          UPpersonnel
        );
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Êtes-vous sûr de vouloir supprimer ce personnel ?")) {
      ...
      try {
        await axios.delete(`https://gestion-hopital-api.vercel.app/personnels/delete/${id}`);
  ...
  return (
    ...
    <div className="min-h-screen bg-[url('/Logo.PNG')] bg-cover flex  justify-center items-center">
      ...
      />
        ...
        {isEditing && UPpersonnel && (
          <div className="mb-6 bg-white p-6 shadow-lg rounded-md">
            <h3 className="text-lg font-semibold mb-4">Modifier le Personnel</h3>
            {/*Formulaire*/}
            {[
              { label: "Nom",value:UPpersonnel.nom, key: "nom" },
              { label: "Code personnel", value:UPpersonnel.codePersonnel, key: "codePersonnel" },
              { label: "Horaires", value: UPpersonnel.horaires, key: "horaires" },
              { label: "Téléphone", value: UPpersonnel.contact, key: "contact" },
            ].map((field) => (
              <div className="mb-4" key={field.key}>
                <label className="block text-sm font-medium text-gray-700">{field.label}</label>
                <input
                  type={field.key === "contact" ? "number" : "text"}
            ...
            <div className="mb-4">
              ...
              <select
                value={UPpersonnel.services}
                onChange={(e) =>
                  setUPpersonnel({ ...UPpersonnel, services: e.target.value })
                }
              ...
              >
                ...
                <option value="Néttoyage">Nettoyage</option>
              </select>
            </div>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700">Catégories</label>
              <select
                value={UPpersonnel.catégories}
                onChange={(e) =>
                  setUPpersonnel({ ...UPpersonnel, catégories: e.target.value })
                }
                className="mt-1 w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
              >
                <option value="" disabled>
                  ...
                  <li key={jour} className="flex items-center">
                    <input
                      ...
                      onChange={() => {
                        ...
                        setUPpersonnel({ ...UPpersonnel, jourService: selectedDays });
                      }}
                      className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <span className="ml-2 text-gray-700">{jour}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <thead className="bg-cyan-500">
              <tr>
                ...
                <th className="py-3 px-4 text-left">Jour Services</th>
                <th className="py-3 px-4 text-left">Horaires</th>
            ...
            <tbody>
              {currentPersonnels.length > 0 ? (
                currentPersonnels.map((personnel) => (
                  <tr key={personnel._id} className="border-b">
                    <td className="py-3 px-4">{personnel.nom}</td>
                    <td className="py-3 px-4">{personnel.codePersonnel}</td>
                    <td className="py-3 px-4">{personnel.services}</td>
                    <td className="py-3 px-4">{personnel.contact}</td>
                    <td className="py-3 px-4">{personnel.jourService.join(", ")}</td>
                    <td className="py-3 px-4">{personnel.horaires}</td>
                    <td className="py-3 px-4 text-center">
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => {setUPpersonnel(personnel);
                            setIsEditing(true);}}
                          className="p-2  rounded-full  group transition-all duration-500  flex item-center"
                        >
                          <svg className="cursor-pointer" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="https
://www.w3.org/2000/svg">
                        ...
                        <button
                          ...
                          className="p-2 rounded-full  group transition-all duration-500  flex item-center"
                        >
                          <svg className="" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="https
://www.w3.org/2000/svg">
